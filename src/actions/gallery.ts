...
export async function getKeepsakesByEventId(eventId: string): Promise<Keepsake[]> {
  try {
    const result = await query(
      'SELECT * FROM keepsakes WHERE event_id = $1 AND hidden = false ORDER BY pinned DESC, created_at DESC',
      [eventId]
    );
