/**
 * Simple logging utility for the Keepsakes application
 * Provides structured logging for guest uploads and other activities
 */
...
class Logger {
  ...
  private createLogEntry(
    ...
    return {
      ...
      eventId,
      eventSlug,
      userAgent: typeof window !== 'undefined' ? window.navigator.userAgent : 'Server Action',
    };
  }

  private async log(entry: LogEntry): Promise<void> {
    console.log('🔍 Logger.log called with entry:', entry);
    ...
    // Store log in Firestore for admin dashboard viewing
    try {
      console.log('💾 Attempting to store log in Firestore...');
      const { storeLog } = await import('./logs');
      await storeLog({
        level: entry.level,
      ...
      // Don't silently fail - we need to see what's wrong
  ...
  async guestUploadStarted(eventSlug: string, eventId: string, fileType: string, fileName?: string): Promise<void> {
    ...
    console.log('🚀 guestUploadStarted called:', { eventSlug, eventId, fileType, fileName });
  ...
  async guestUploadCompleted(eventSlug: string, eventId: string, keepsakeId: string, fileType: string, fileName?: string): Promise<void> {
    ...
    console.log('✅ guestUploadCompleted called:', { eventSlug, eventId, keepsakeId, fileType, fileName });
    await this.log(this.createLogEntry(
      'info',
      'upload',
  ...
  async guestUploadFailed(eventSlug: string, eventId: string, error: string, fileType?: string, fileName?: string): Promise<void> {
    ...
    console.log('❌ guestUploadFailed called:', { eventSlug, eventId, error, fileType, fileName });
    await this.log(this.createLogEntry(
      'error',
      'upload',
      'Guest upload failed',
